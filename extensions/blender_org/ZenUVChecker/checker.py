...
from .utils import switch_shading_style
from .files import load_checker_image
from .ui import zen_message
from .constants import (
    ADDON_SHORT,
    ZEN_IMAGE_NODE_NAME,
    ZEN_GENERATED_IMAGE_NAME,
    ZEN_GLOBAL_OVERRIDER_NAME,
    ZEN_GLOBAL_OVERRIDER_NAME_OLD,
    ZEN_OVERRIDER_NAME,
    ZEN_GENERIC_MAT_NAME,
    ZEN_NODE_COLOR,
    ZEN_TILER_NODE_NAME,
...
def ensure_material_slots(context, _obj):
...
def ensure_user_mats_consistency(context, _objs):
    """ Refine and repair user materials from selected objects """
    for _obj in _objs:
        for _slot in ensure_material_slots(context, _obj):
            ensure_material(_slot)
...
def implement_zen_overrider(context, _obj, _GlobalOverrider):
    ...
    material_slots = ensure_material_slots(context, _obj)

    for slot in material_slots:
        ensure_material(slot)
...
def reset_variables(context):
    ...
    props.tex_checker_interpolation = True
    props.tex_checker_tiling = (1.0, 1.0)
    props.tex_checker_offset = 0.0
...
class ZTCHK_OT_CheckerToggle(bpy.types.Operator):
    """ Zen UV Checker Processor """
    bl_idname = f"view3d.{ADDON_SHORT}_toggle"
    ...
    def draw_toggled(cls, layout: bpy.types.UILayout, context: bpy.types.Context):
        ...
        layout.operator(
            ...
            icon_value=icon_get("checker_32"))
    ...
    def execute(self, context):
        from .ui import zen_message
        from .utils import get_current_shading_style, resort_by_type_mesh_in_edit_mode_and_sel
        from .preferences import get_scene_props
        ...
        if GlobalOverrider is False:
            ...
            return {'CANCELLED'}
        ensure_user_mats_consistency(context, objs)
        materials_with_overrider = get_materials_with_overrider(get_materials_from_objects(context, objs))

        bpy.app.driver_namespace[ZTCHK_OT_CheckerToggle.LITERAL_TOGGLE] = False
        ...
        if materials_with_overrider:  # Case checker exist in currently selected objects - Turn Off Checker
            ...
            prev_color_type = addon_scene_props.prev_color_type
            ...
            if context.space_data.type == 'IMAGE_EDITOR':
                was_image_name = bpy.app.driver_namespace.get(
                    ZTCHK_OT_CheckerToggle.LITERAL_PREV_IMAGE, '')
                if was_image_name:
            ...
            if context.space_data.type == 'IMAGE_EDITOR':
                ...
                bpy.app.driver_namespace[ZTCHK_OT_CheckerToggle.LITERAL_PREV_IMAGE] = was_image.name if was_image else ''

            p_prev_type = get_current_shading_style(context)
            if p_prev_type is not None:
                addon_scene_props.prev_color_type = p_prev_type
            ...
            if p_image is not None:
                ...
                bpy.app.driver_namespace[ZTCHK_OT_CheckerToggle.LITERAL_TOGGLE] = True
...
class ZTCHK_OT_Reset(bpy.types.Operator):
    """ Zen UV Checker Reset """
    bl_idname = f"view3d.{ADDON_SHORT}_reset"
    bl_label = "Reset Checker"
    bl_description = "Reset Zen UV Checker to Default state "
    bl_options = {'REGISTER', 'UNDO'}
...
class ZTCHK_OT_Remove(bpy.types.Operator):
    """ Zen UV Checker Remove """
    bl_idname = f"view3d.{ADDON_SHORT}_remove"
    bl_label = "Remove Checker Nodes"
    bl_description = "Remove Zen UV Checker Nodes from the scene materials"
    bl_options = {'REGISTER', 'UNDO'}
...
class ZTCHK_OT_OpenEditor(bpy.types.Operator):
    """ Zen UV Checker Remove """
    bl_idname = f"view3d.{ADDON_SHORT}_open_editor"
    bl_label = "Open Shader Editor"
    bl_description = "Open Shader Editor with Zen UV Checker Node"
    bl_options = {'REGISTER', 'UNDO'}

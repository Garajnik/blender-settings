...
from .. utils.group import get_group_default_name, group, ungroup, get_group_matrix, select_group_children, get_child_depth, clean_up_groups, fade_group_sizes
...
from .. utils.object import parent, unparent, compensate_children, get_eval_bbox
...
class Group(bpy.types.Operator):
    ...
    def group(self, context, objects, ungroupable, debug=False):
        ...
        draw_fading_label(context, text=text, x=self.coords.x, y=self.coords.y, color=color, alpha=alpha, time=time)
...
class ToggleGroupMode(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if space.use_filter_object_mesh:
            ...
            if get_prefs().group_auto_name:
                if (suffix := get_prefs().group_suffix) and (prefix := get_prefs().group_prefix):
                    space.filter_text = f"{prefix}*{suffix}"

                elif suffix := get_prefs().group_suffix:
                    space.filter_text = f"*{suffix}"

                elif prefix := get_prefs().group_prefix:

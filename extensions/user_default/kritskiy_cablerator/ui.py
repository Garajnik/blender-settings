...
def draw_callback_px(self, context):
    ...
    vertices = (
        (pos_x - 10, pos_y - bottom_add), (pos_x + column_width + add_width + 10, pos_y - bottom_add),
        (pos_x - 10, pos_y + height), (pos_x + column_width + add_width + 10, pos_y + height))
    indices = (
        (0, 1, 2), (2, 1, 3))
    self.ui_vertices = vertices
    self.ui_indices = indices
    self.ui_shader = create_2d_shader()
    self.ui_batch = batch_for_shader(self.ui_shader, 'TRIS', {"pos": self.ui_vertices}, indices=self.ui_indices)
    self.ui_shader.bind()

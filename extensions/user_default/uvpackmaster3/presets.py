...
class UVPM3_OT_SavePresetBase(Operator):
...
class UVPM3_OT_LoadPresetBase(Operator):
...
class ScenePropsPresetLoader(PresetLoader):
...
class UVPM3_OT_LoadScenePropsPreset(UVPM3_OT_LoadPresetBase):

    LOADER_TYPE = ScenePropsPresetLoader

    def get_target(self):
        return self.scene_props


class UVPM3_OT_LoadPreset(LoadPresetInvokeHelper, UVPM3_OT_LoadScenePropsPreset, ImportHelper, PresetFilenameMixin):
...
class TargetBoxPresetLoader(ScenePropsPresetLoader):
...
class UVPM3_OT_LoadTargetBox(LoadPresetInvokeHelper, UVPM3_OT_LoadScenePropsPreset, ImportHelper, PresetFilenameMixin):
...
class ResetToDefaultsPresetLoader(ScenePropsPresetLoader):
...
class UVPM3_OT_ResetToDefaults(OpConfirmationMsgMixin, UVPM3_OT_LoadScenePropsPreset):

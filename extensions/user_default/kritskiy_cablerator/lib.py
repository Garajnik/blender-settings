...
def main(context, event, self, batch):
    ...
    def obj_ray_cast(obj, matrix):
        ...
        if success:
            ...
            center = mesh_data.polygons[face_index_scene].center
            return matrix_inv @ location_scene, normal_scene @ matrix, face_index_scene, obj, matrix, normal_scene, matrix @ center
        else:
    ...
    if bpy.context.space_data.local_view:
        for ob in bpy.context.view_layer.objects:
          if not ob.visible_get(view_layer=None): to_hide.append(ob)

...
class ZUV_OT_Select_UV_Borders(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if ZenPolls.version_since_3_2_0 and area == 'IMAGE_EDITOR':
            for obj in objs:
            ...
            for obj in objs:
                ...
                if self.mode == 'FACE':
                    for face in bm.faces:
                        face.select = condition[self.condition](UvFaceArea.get_uv_faces_area([face, ], uv_layer) * FACE_UV_AREA_MULT, self.sc_lv_prop.area_value_for_sel)
                else:

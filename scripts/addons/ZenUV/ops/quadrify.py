...
from ZenUV.utils.generic import fit_uv_view, pin_island, resort_objects_by_selection, select_all
...
class ZUV_OT_Quadrify(bpy.types.Operator):
    ...
    def is_pack_allowed(self, context):
        ...
        return context.scene.zen_uv.op_quadrify_props.packAfQuadrify
    ...
    def execute(self, context):
        ...
        objects = resort_objects_by_selection(context, context.objects_in_mode)

        if not objects:
            self.report({'WARNING'}, "There are no selected objects")
            return {'CANCELLED'}
        # if objects:
        if props.post_td_mode == 'AVERAGED':
            # Get Init TD
            init_td = 0.0
            for obj in objects:
                bm = bmesh.from_edit_mesh(obj.data)
                uv_layer = bm.loops.layers.uv.verify()
                init_td += TexelDensityFactory.calc_averaged_td(obj, uv_layer, [bm.faces, ], TdContext(context))[0]
            init_td = init_td / len(objects)
        elif props.post_td_mode == 'GLOBAL_PRESET':
            init_td = context.scene.zen_uv.td_props.td_global_preset
        else:
            init_td = None
